...
def resolve_project_path(project_name: str, config: dict) -> Path:
    ...
    # the project name so symlinked workspace dirs are passed to Godot as
    # the realpath. Without this, Godot may load the project but resolve
    # the symlink itself, occasionally leading to stale project cache state
    # when the same project is mounted via
