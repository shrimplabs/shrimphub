...
def _agent_status() -> Dict[str, Any]:
    """Build the agents status dict used by both GET and POST responses."""
    return {
        "gardener": {
...
def register_routes(app, config: Dict,
    ...
    def get_meta_mode():
        """Return current state of the master meta-mode toggle and agent statuses."""
        return jsonify({
...
def _cascade_to_schedulers(meta_mode_enabled: bool) -> None:
    ...
    # (via orchestrator globals) inside the fired closure. The actual task
    # creation is guarded by orchestrator.META_MODE_ENABLED.
    pass
