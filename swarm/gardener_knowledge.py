...
Auto-generated markdown view (RENDERED ONLY, never written to disk):
  data/gardener_patterns.md

The render_markdown() function returns the markdown as a string. No code in this
module writes it to disk. Use it explicitly via `data/gardener_patterns.md`.

Note: data/SWARM_KNOWLEDGE.md is exclusively owned by the unified chat
write_swarm_memory tool (human/chat-authored). It is NEVER overwritten by this
module. The two files are injected as separate memory sections by
swarm.api_chat._load_unified_memory().
"""
...
# Owned by: render_markdown() output path -- auto-generated, NOT written to disk by any
# function in this module. data/SWARM_KNOWLEDGE.md is owned by api_chat.write_swarm_memory.
MARKDOWN_PATH = _DATA_DIR / "gardener_patterns.md"
...
def render_markdown(data_dir: Path | None = None) -> str:
    ...
    lines = [
        ...
        "     Target path: data/gardener_patterns.md (rendered, never auto-written).",
        "     ============================================================ -->",
        "",
        "# Gardener Patterns",
        "",
