...
## Interactive Task Work (Codex)

You can pick up and complete swarm tasks yourself instead of delegating to background agents. Use the `/swarm-task` skill (symlinked from `skills/swarm-task/SKILL.md` into `~/.Codex/skills/`):
...
swarm_mcp_server.py          MCP server exposing swarm API as tools (register in ~/.Codex/settings.json)
...
| `meta_investigation_provider` | `""` | LLM provider used for investigation calls (defaults to the main provider; set to `"Codex"` etc. to override) |
...
| `Codex` | `anthropic_native` (x-api-key) | `ANTHROPIC_API_KEY` | Codex-sonnet-4-6 |
| `openrouter` | `openai` | `OPENROUTER_API_KEY` | Codex-3.5-sonnet |
