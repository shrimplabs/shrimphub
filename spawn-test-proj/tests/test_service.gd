...
# GUT test suite for the Python HTTP service.
# Exercises: GET /ping, GET /health, POST /spawn
...
func _http_get(req_path: String) -> Dictionary:
	...
	err = http.request(HTTPClient.METHOD_GET, req_path, [])  # [] = no custom headers
	if err != OK:
...
func _http_post(req_path: String, body: String) -> Dictionary:
	...
	if http.get_status() != HTTPClient.STATUS_CONNECTED:
		return {"error": "status: " + str(http.get_status())}
	var raw_body: PackedByteArray = body.to_utf8_buffer()
	err = http.request(HTTPClient.METHOD_POST, req_path, [], raw_body)
	if err != OK:
		return {"error": "request failed: " + str(err)}
	while http.get_status() == HTTPClient.STATUS_REQUESTING:
		http.poll()
...
func test_service_starts() -> void:
	assert_true(_start_service(), "service.py starts without error")
	assert_gt(_service_pid, 0, "service PID is positive")

func test_get_ping_returns_200() -> void:
	assert_true(_start_service(), "service must start first")
	var resp := _http_get("/ping")
	assert_false(resp.has("error"), "no connection error: " + str(resp.get("error", "")))
...
func test_spawn_increments_id() -> void:
	assert_true(_start_service(), "service must start first")
	var r1 := _http_post("/spawn", "{}")
	var r2 := _http_post("/spawn", "{}")
